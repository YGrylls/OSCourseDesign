...
PRIVATE struct inode * new_inode(int dev, int inode_nr, int start_sect,int mode);
...
PUBLIC int do_open()
{
	...
	printl("@n >%s\n", pathname);//test
	int stripres = strip_path(filename, pathname, &dir_inode);
	printl("@n >%s\n", filename);//test
	printl("@ino_nr > %d\n", inode_nr);//test
...
//			strip_path(filename, pathname, &dir_inode);
			dir_open = pin;
			memset(open_name, 0, 12);
			memcpy(open_name, filename,12);
//			printl("@pin mark1");//test
			dir_list(dir_open, dir_ls); //refresh
...
//			else if (imode == I_DIRECTORY) {						//open directory
...
PRIVATE struct inode * create_file(char * path, int flags)
{
	assert(flags & (O_CREAT | O_MKDR));
	char filename[MAX_PATH];
	struct inode * dir_inode;
	int mode = I_REGULAR;
	if (flags & O_MKDR) {
		mode = I_DIRECTORY;
		printl("Creating directory...\n");// test use
	}
	...
	if (newino->i_mode == I_DIRECTORY) {
		new_dir_entry(newino, newino->i_num, ".");  //add it to itself
		new_dir_entry(newino, dir_inode->i_num, ".."); //add its father to it
	}
	return newino;
}
...
PRIVATE void new_dir_entry(struct inode *dir_inode,int inode_nr,char *filename)
{
	if (strcmp(filename, ".")==0) {
		assert(dir_inode->i_num == inode_nr); //check if dir
	}
	/* write the dir_entry */
	...
	for (i = 0; i < nr_dir_blks; i++) {
		memset(fsbuf, 0, FSBUF_SIZE);
		RD_SECT(dir_inode->i_dev, dir_blk0_nr + i);

		pde = (struct dir_entry *)fsbuf;  //get a dir entry
...
PUBLIC int do_ls() {
	int ret = dir_list(dir_open, dir_ls);
	return ret;
}