...
PUBLIC int do_unlink()
{
	...
	if ((pin->i_mode != I_REGULAR)&&(pin->i_mode!=I_DIRECTORY)) { /* can only remove regular files */
		printl("cannot remove file %s, because "                                    //now we can also remove directory
	...
	if ((pin->i_mode== I_REGULAR) && (pin->i_cnt > 1)) {	/* the file was opened */
	...
	if (pin->i_mode == I_DIRECTORY) {
		...
		if (nr_dir_entries > 2) {
			put_inode(pin);
			return -1;				//only empty directory can be removed
		}
	}
