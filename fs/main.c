...
//	mkfs();
	...
	if (sb == 0) {
		printl("No File System Found On the Disk, Creating a New One \n");  //create the FS on the disk if there's not one
		mkfs();
	}
	else {
		printl("Fils System Found \n");   //FS found
	}
//	assert(sb->magic == MAGIC_V1);

	root_inode = get_inode(ROOT_DEV, ROOT_INODE);

	dir_open = root_inode;  //the default directory is the root directory
	dir_list(dir_open, dir_ls);
...
PRIVATE void mkfs()
{
	...
	pde->inode_nr = 1;
	strcpy(pde->name, ".");               //'.' points to the directory itself './'
